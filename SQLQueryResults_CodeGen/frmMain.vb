...
Public Class frmMain
    ...
    Private Sub cmdCnnString_Click(sender As Object, e As EventArgs) Handles cmdCnnString.Click
        Dim lDataUI As New ConnectionUI.DataConnectionDialog()

        Try
            With lDataUI
                .DataSources.Add(ConnectionUI.DataSource.SqlDataSource)
                .DataSources.Add(ConnectionUI.DataSource.AccessDataSource)
                .DataSources.Add(ConnectionUI.DataSource.SqlFileDataSource)

                .SelectedDataSource = ConnectionUI.DataSource.SqlDataSource
                .SelectedDataProvider = ConnectionUI.DataProvider.SqlDataProvider

                If Not String.IsNullOrEmpty(txtConnectionString.Text) Then
                    .ConnectionString = txtConnectionString.Text.Trim
                End If
            End With
            If ConnectionUI.DataConnectionDialog.Show(lDataUI) = DialogResult.OK Then
                txtConnectionString.Text = lDataUI.ConnectionString
            End If

        Catch ex As Exception
            MsgBox(ex.Message, vbOKOnly + vbExclamation, My.Application.Info.Title)
        Finally
            lDataUI.Dispose()
            lDataUI = Nothing
        End Try
